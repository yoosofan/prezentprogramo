...
class Template(object):
        ...
        for key, files in self.config.items():
            ...
            if key.startswith("css"):
                ...
                for filename in files.split():
                    self.add_resource(
                        filename, CSS_RESOURCE, extra_info=media, is_in_template=True
                    )
    ...
    def get_source_path(self, resource):
        ...
        if self.builtin_template:
            return os.path.join(
                HOVERCRAFT_DIR, self.template.strip("/"), resource.filepath
            )
    ...
    def copy_resource(self, resource, targetdir):
        ...
        if resource.resource_type == DIRECTORY_RESOURCE:
            for file_path in glob.iglob(
                os.path.join(source_path, "**"), recursive=True
            ):
    ...
    def _copy_file(self, source_path, target_path):
        ...
        if os.path.exists(target_path) and os.path.getmtime(
            source_path
